...
export default function InventoryTable({ items, onUpdateStock, onFilterChange }: InventoryTableProps) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="p-4 border-b border-slate-200 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <select
            className="text-xs bg-slate-50 border-slate-200 rounded-lg py-1 px-3 outline-none"
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <span>Showing 1-{Math.min(10, items.length)} of {items.length} items</span>
            ...
            {items.map((item) => (
              <tr key={item.id} className={item.status === 'low' ? 'bg-red-50 border-l-4 border-l-red-500' : ''}>
                ...
                <td className="px-6 py-4 text-center">
                  <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-bold uppercase ${item.status === 'low' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                    }`}>
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-2">
                    <button
                      ...
                      className="size-8 flex items-center justify-center bg-white border border-slate-200 rounded-lg text-primary hover:bg-primary hover:text-white transition-colors"
                    >
                      <Plus size={16} />
                    </button>
                    <button
                      onClick={() => onUpdateStock(item.id, -1)}
                      className="size-8 flex items-center justify-center bg-white border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-100 transition-colors"
                    >
                      <Minus size={16} />
                    </button>
                  </div>
